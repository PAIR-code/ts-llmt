...
export type IfNeverElse<T, IfNeverT, ElseT> = [T] extends [never]
  ? IfNeverT
  : ElseT;
...
export function matchTemplate<Ns extends string>(
  ...
  if (matchPrefix) {
    const prefixMatch = match.finalStr.match(
      `^${escapeStringInMatch(parts.prefix)}`
    );
    if (prefixMatch) {
...
export class TemplVar<Ns extends string, N extends Ns> {
  ...
  substStr(replacement: string): Template<Exclude<Ns, N>> {
    const newTemplate = this.rawVariable.subst(
      this.template.escaped,
      replacement
    );
    const newVarList = [
...
export class Template<Ns extends string> {
  ...
  substs<S extends Ns, N2s extends string>(replacements: {
    ...
    Exclude<Ns, keyof typeof replacements> | (string extends N2s ? never : N2s)
  > {
    ...
    for (const k of Object.keys(replacements) as S[]) {
      ...
      if (typeof r === 'string') {
        ...
        newTempl = (newTempl as unknown as Template<Ns>).vars[
          k
  ...
  concat<N2s extends string>(secondPart: Template<N2s>): Template<Ns | N2s> {
    const vars = [...this.varList(), ...secondPart.varList()] as NamedVar<
      Ns | N2s
    >[];
...
type TemplateArgName<T> = T extends
  | NamedVar<infer Hs>
  | Template<infer Hs>
  | string
  ? Hs
